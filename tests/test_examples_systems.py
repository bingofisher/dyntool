...
def test_scenario_custom_extension(tmp_path: Path) -> None:
    result = run_example(
        "examples/10_scenarios/08_custom_extension/main.py",
        output_dir=tmp_path / "custom_extension",
    )
    assert result["registered_model_class"] == "JerkSeries"
    assert result["sample_count"] == 1
    assert Path(result["model_path"]).exists()
    assert Path(result["sample_set_dir"]).exists()


def test_scenario_docs_exist() -> None:
    for path in [
        "docs/examples_overview.md",
        "docs/systems/11_custom_extension.md",
