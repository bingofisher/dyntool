...
from ._sample_state import _SampleCategoryState, _SampleIdentityState
...
class SampleBase(BaseModel):
    ...
    def _normalize_public_categories(
        ...
        return _SampleCategoryState(self).normalize_public_categories(categories)

    def _default_alias_for_metadata(self, metadata: MetadataBase | None = None) -> str:
        """根据元数据计算默认 alias。"""

        return _SampleIdentityState(self).default_alias_for_metadata(metadata)

    def _set_alias_internal(self, alias: str, *, mark_override: bool) -> None:
        """内部设置 alias，并按需维护覆盖标记。"""

        _SampleIdentityState(self).set_alias_internal(alias, mark_override=mark_override)

    def _restore_alias_internal(self, alias: str | None) -> None:
        """按持久化结果恢复 alias，并推导是否属于手工覆盖。"""

        _SampleIdentityState(self).restore_alias_internal(alias)
    ...
    def _bind_metadata(self, metadata: MetadataBase) -> None:
        ...
        _SampleIdentityState(self).bind_metadata(metadata)
    ...
    def _sync_alias_after_metadata_change(
        ...
        *,
        ...
        _SampleIdentityState(self).sync_alias_after_metadata_change(
            old_uid=old_uid,
    ...
    def _sync_identity_state(
        ...
        *,
        ...
        _SampleIdentityState(self).sync_identity_state(
            old_uid=old_uid,
            old_metadata_alias=old_metadata_alias,
    ...
    def _set_storage_presence_internal(
        ...
        _SampleCategoryState(self).set_storage_presence(presence)
