...
- 当前目录对应 `v1.2.0` 的 breaking/RC 版本线。
- 该版本线用于承接兼容层清理、结构收敛和迁移口径更新。
- 主目录 `AdvDynTool` 保持 `main` 稳定线，供现有外部项目继续直接引用。
...
- 正式样本集对象继续保留对象级稳定入口，并通过 `SampleSetBase` 上的薄委托把统计导出和报告包导出交给 `dyntool.reporting`
